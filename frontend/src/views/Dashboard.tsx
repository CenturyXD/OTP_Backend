...
import IpFormModal, { type IpFormData } from '../components/IpFormModal';
...
const Dashboard: React.FC = () => {
    ...
    const [editingRecord, setEditingRecord] = useState<CoreIpData | null>(null);
    ...
    const fetchSummaryData = async () => {
        try {
            const response = await ipService.getCoreIps(1, dbTotal > 0 ? dbTotal : 10000, '');
    ...
    const handleEdit = (record: CoreIpData) => {
    ...
    const exportData = async (dataPromise: Promise<any>, type: 'All' | 'Current') => {
        ...
        try {
            ...
            const dataToExport = response.data.map((item: CoreIpData) => ({
    ...
    const handleExportExcel = () => {
        Modal.confirm({
            ...
            onOk: () => exportData(ipService.getCoreIps(1, dbTotal > 0 ? dbTotal : 10000, ''), 'All'),
    ...
    return (
        <>
            <Title level={2}>Core IP Management</Title>
            <Text type="secondary">Overview of your core network IPs.</Text>
