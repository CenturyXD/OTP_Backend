...
import type { IntraData } from '../api/ApiClient';
...
const IpIntraPage: React.FC = () => {
    const { notification } = App.useApp();

    // --- States ---
    const [data, setData] = useState<IntraData[]>([]);
    ...
    const [editingRecord, setEditingRecord] = useState<IntraData | null>(null);

    // --- Data Fetching ---
    const fetchData = async (page: number, pageSize: number, search: string) => {
        setLoading(true);
        try {
            const response = await ipService.getIntraIps(page, pageSize, search);
    ...
    const fetchSummaryData = async () => {
        try {
            const response = await ipService.getIntraIps(1, dbTotal > 0 ? dbTotal : 10000, '');
    ...
    // --- Handlers ---
    const handleTableChange: TableProps<IntraData>['onChange'] = (newPagination) => {
        setPagination({
            ...pagination,
    ...
    const handleEdit = (record: IntraData) => {
    ...
    const handleFormFinish = async (values: IpBrkFormData) => {
        const action = editingRecord && editingRecord.id !== undefined
            ? ipService.updateIntraIps(editingRecord.id, { ...editingRecord, ...values })
            : ipService.createIntraIps(values);
    ...
    const exportData = async (dataPromise: Promise<any>, type: 'All' | 'Current') => {
        ...
        try {
            ...
            const dataToExport = response.data.map((item: IntraData) => ({
            ...
            exportService.toExcel(dataToExport, `Intra_IPs_Export_${type}`);
    ...
    const handleExportExcel = () => {
        Modal.confirm({
            ...
            onOk: () => exportData(ipService.getIntraIps(1, dbTotal > 0 ? dbTotal : 10000, ''), 'All'),
            onCancel: () => console.log('Export cancelled'),
        });
    };

    const columns: TableProps<IntraData>['columns'] = [
    ...
    return (
        <>
            <Title level={2}>Intra IP Management</Title>
            <Text type="secondary">Overview of your Intra IPs.</Text>
