...
export type IntraFormData = Omit<IntraData, 'id' | 'created_at' | 'updated_at'>;

interface IpFormModalProps {
    visible: boolean;
    loading: boolean;
    initialData: IntraData | null; // รับข้อมูลเริ่มต้น (ถ้ามีคือโหมด Edit)
    onCancel: () => void;
    onFinish: (values: IntraFormData) => void;
    onDelete?: () => void; // ฟังก์ชันสำหรับลบ (optional)
}

const BrkIpFormModal: React.FC<IpFormModalProps> = ({ visible, loading, initialData, onCancel, onFinish, onDelete }) => {
